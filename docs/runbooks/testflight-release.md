...
| EAS environments    | Default EAS environment selection follows the profile names `development`, `preview`, and `production`.                                                                       | Configure the three public mobile variables in each matching EAS environment; do not confuse these names with `APP_ENV`.                           |
...
| Public mobile env   | `EXPO_PUBLIC_API_BASE_URL`, `EXPO_PUBLIC_MOCK_MODE`, and default-false `EXPO_PUBLIC_HEALTHKIT_ENABLED` are the only `EXPO_PUBLIC_*` variables.                                | They are readable from the app bundle and must never contain credentials or secret endpoints/tokens.                                               |
| Native generation   | No `ios/` or `android/` directory is checked in; Expo config plugins include the CU-097 read-only HealthKit plugin plus Dev Client and Router.                                | EAS uses Expo prebuild/CNG; any plugin or native-module change requires compatibility review and a new binary.                                     |
| Native dependencies | Expo Dev Client plus the CU-097 pinned HealthKit/Nitro scaffold and existing Router, SQLite, MMKV, Gesture Handler, Reanimated, Safe Area Context, and Screens are installed. | A newly rebuilt dev client is mandatory. HealthKit remains default-off, read-only, explicit-action, and subject to `healthkit-live-validation.md`. |
...
| Build profile | EAS environment | `APP_ENV` in `eas.json` | `EXPO_PUBLIC_API_BASE_URL`               | `EXPO_PUBLIC_MOCK_MODE`                                        | `EXPO_PUBLIC_HEALTHKIT_ENABLED`                           |
| ------------- | --------------- | ----------------------- | ---------------------------------------- | -------------------------------------------------------------- | --------------------------------------------------------- |
| `development` | `development`   | `dev`                   | Approved dev API URL                     | Explicitly `true` only for labelled mock QA; otherwise `false` | `false` until the HealthKit Phase Z checklist is approved |
| `preview`     | `preview`       | `staging`               | Approved staging API URL                 | `false` for a live candidate                                   | `false`                                                   |
| `production`  | `production`    | `prod`                  | Approved private-beta production API URL | `false`                                                        | `false`                                                   |
...
- [ ] **Phase Z manual:** Confirm native assets and required usage descriptions are present for the
      features actually compiled into the binary. Follow `healthkit-live-validation.md`; keep the
      HealthKit flag off until its native and physical-device gates pass.
