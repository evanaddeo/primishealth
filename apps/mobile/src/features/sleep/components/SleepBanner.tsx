/**
 * SleepBanner — calm, non-blocking provisional/stale notice (CU-063).
 *
 * Surfaces a provisional ("early read") or stale ("waiting on sync") state
 * without alarm (UX-AC-HOME-004 spirit). Status is conveyed by both a dot color
 * AND text, never color alone (UX-COLOR-001).
 */
...
export interface SleepBannerProps {
  banner: SleepBannerVm;
  testID?: string;
}

export function SleepBanner({ banner, testID }: SleepBannerProps): React.JSX.Element {
