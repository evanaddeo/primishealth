...
/**
 * Stores a computed Primis score for a user on a specific date.
 *
 * Do NOT insert rows here unless the scoring engine (Phase F) has computed them.
 * One row per (user_id, score_type, local_date, algorithm_version) — the unique
 * constraint enables idempotent upserts when the scoring engine re-runs.
 *
 * score_type allowed values (§16.1 / @primis/core-types ScoreType):
 *   'sleep_score' | 'recovery_score' | 'training_readiness_score' |
 *   'strain_score' | 'nutrition_score' | 'wellbeing_score' |
 *   'bedtime_adherence_score'
 *
 * score_band: aligns with @primis/core-types ScoreBand.
 *   Allowed: 'poor' | 'low' | 'fair' | 'good' | 'excellent'
 *
 * primary_drivers: serialized ScoreDriverDto[] (see @primis/api-contracts).
 * missing_inputs: serialized MissingMetricDto[] (see @primis/api-contracts).
 */
export interface ScoreSnapshotsTable {
  ...
  user_id: string;
  /**
   * Which Primis score type this snapshot represents.
   * Enforced by DB CHECK constraint; matches core-types ScoreType values.
   */
  score_type: string;
  /** Local calendar date (user's timezone) for which the score was computed. */
  local_date: string;
  /** IANA timezone string used for this computation. */
  timezone: string;
  /** Final composite score 0–100.00 (two decimal precision). */
  score_value: string;
  /** Qualitative band; aligns with @primis/core-types ScoreBand. */
  score_band: NullableCol<string>;
  /** Scoring algorithm version string (e.g. '1.0.0'). */
  ...
  /** Percentage of expected input data that was available (0–100.00). */
  data_coverage_pct: NullableCol<string>;
  /** Confidence score (0.0000–1.0000). */
  confidence_score: NullableCol<string>;
  /** Serialized ScoreDriverDto[] array — top factors influencing the score. */
  primary_drivers: Generated<unknown[]>;
  /** Serialized MissingMetricDto[] array — metrics absent during scoring. */
  missing_inputs: Generated<unknown[]>;
  /** Free-form extension metadata. */
  metadata: Generated<Record<string, unknown>>;
}

/** Full row shape returned by SELECT queries on `score_snapshots`. */
export type ScoreSnapshot = Selectable<ScoreSnapshotsTable>;
/** Shape accepted by INSERT queries on `score_snapshots`. */
export type NewScoreSnapshot = Insertable<ScoreSnapshotsTable>;
/** Shape accepted by UPDATE queries on `score_snapshots`. */
...
/**
 * Per-component breakdown for a score snapshot.
 *
 * Cascade-deleted when the parent `score_snapshots` row is deleted. Upserting
 * a new snapshot for the same (user, score_type, local_date, algorithm_version)
 * removes and replaces component rows automatically via the CASCADE.
 *
...
export interface ScoreComponentValuesTable {
  ...
  user_id: string;
  /**
   * Short stable identifier for this component.
   * Examples: 'hrv_vs_baseline', 'sleep_debt', 'rhr_delta', 'deep_sleep_pct'
   */
  component_code: string;
  /** Human-readable label for UI display. */
  component_label: string;
  /** Raw input value before normalization. */
  raw_value: NullableCol<number>;
  /** Normalized value (generally 0–1 or −1–1 depending on algorithm). */
  normalized_value: NullableCol<string>;
  /** Pre-computed weighted contribution to the composite score. */
  weighted_contribution: NullableCol<string>;
  /** Weight of this component (0–1); original value before renormalization. */
  weight: NullableCol<string>;
  /** Canonical unit string for raw_value, if applicable. */
  unit: NullableCol<string>;
  /** Scoring directionality: 'positive' | 'negative' | 'neutral'. */
  direction: NullableCol<string>;
  /** Human-readable explanation for this component's contribution. */
  explanation: NullableCol<string>;
  /** Free-form extension metadata. */
  metadata: Generated<Record<string, unknown>>;
}

/** Full row shape returned by SELECT queries on `score_component_values`. */
export type ScoreComponentValue = Selectable<ScoreComponentValuesTable>;
/** Shape accepted by INSERT queries on `score_component_values`. */
export type NewScoreComponentValue = Insertable<ScoreComponentValuesTable>;
/** Shape accepted by UPDATE queries on `score_component_values`. */
...
/**
 * Audit log for scoring engine executions.
 *
 * Useful for debugging and reprocessing (ALG-PRIN-008). Not required for
 * day-to-day reads; joins to score_snapshots via metadata or time window.
 *
 * run_type: 'daily_scores' | 'backfill' | 'reprocess' | 'manual' | 'experiment'
 * status: 'running' | 'succeeded' | 'failed' | 'partial_success'
 */
...
/** Full row shape returned by SELECT queries on `algorithm_runs`. */
export type AlgorithmRun = Selectable<AlgorithmRunsTable>;
/** Shape accepted by INSERT queries on `algorithm_runs`. */
export type NewAlgorithmRun = Insertable<AlgorithmRunsTable>;
/** Shape accepted by UPDATE queries on `algorithm_runs`. */
...
/**
 * Deterministic insights before optional AI wording.
 *
 * `structured_summary` is the authoritative content. `natural_language_summary`
 * is populated by the AI layer in Phase I and may be null.
 *
...
export interface InsightCandidatesTable {
  ...
  /**
   * Insight category.
   * Examples: 'recovery_driver' | 'sleep_pattern' | 'training_load' |
   *   'nutrition_correlation' | 'anomaly' | 'recommendation'
   */
  insight_type: string;
  /** Primary date this insight applies to (user's local timezone). */
  local_date: NullableCol<string>;
  /** Start date for range-spanning insights. */
  start_date: NullableCol<string>;
  /** End date for range-spanning insights. */
  ...
  /** Short headline shown in the app. */
  title: string;
  /** Machine-readable structured content for AI context and explainability. */
  structured_summary: Record<string, unknown>;
  /** AI-generated natural language copy; null until Phase I populates it. */
  natural_language_summary: NullableCol<string>;
  /** Actionable recommendation text; null if no action is appropriate. */
  ...
  /** UUIDs of score_snapshots rows that contributed to this insight. */
...
/** Full row shape returned by SELECT queries on `insight_candidates`. */
export type InsightCandidate = Selectable<InsightCandidatesTable>;
/** Shape accepted by INSERT queries on `insight_candidates`. */
export type NewInsightCandidate = Insertable<InsightCandidatesTable>;
/** Shape accepted by UPDATE queries on `insight_candidates`. */
...
/**
 * User-specific statistical correlation findings.
 *
 * Do NOT present correlations as conclusions until sample size and confidence
 * thresholds are met. UI/AI should say "appears associated" rather than "caused."
 *
 * method: 'simple_difference' | 'pearson' | 'spearman' |
 *   'lagged_difference' | 'regression_lite'
...
export interface CorrelationResultsTable {
  ...
  /**
   * Factor being tested.
   * Examples: 'late_caffeine', 'alcohol', 'sleep_duration', 'training_load'
   */
  factor_code: string;
  /** Canonical metric code of the outcome variable. */
  outcome_metric_code: string;
  window_start_date: string;
  window_end_date: string;
  lag_days: Generated<number>;
  sample_size: number;
  effect_size: NullableCol<string>;
  correlation_value: NullableCol<string>;
  p_value: NullableCol<string>;
  /** confidence_level: low | medium | high */
...
/** Full row shape returned by SELECT queries on `correlation_results`. */
export type CorrelationResult = Selectable<CorrelationResultsTable>;
/** Shape accepted by INSERT queries on `correlation_results`. */
export type NewCorrelationResult = Insertable<CorrelationResultsTable>;

// ---------------------------------------------------------------------------
// anomaly_events (000006_outputs_and_dashboard.sql — §17.3)
// ---------------------------------------------------------------------------

/**
 * Detected statistical deviations from the user's personal baseline.
 *
...
export interface AnomalyEventsTable {
  ...
  metric_code: string;
  local_date: string;
  observed_value: NullableCol<number>;
  expected_value: NullableCol<number>;
  z_score: NullableCol<string>;
  /** severity: low | medium | high */
  severity: string;
  /** status: active | dismissed | resolved */
  status: Generated<string>;
...
/** Full row shape returned by SELECT queries on `anomaly_events`. */
export type AnomalyEvent = Selectable<AnomalyEventsTable>;
/** Shape accepted by INSERT queries on `anomaly_events`. */
export type NewAnomalyEvent = Insertable<AnomalyEventsTable>;
/** Shape accepted by UPDATE queries on `anomaly_events`. */
export type AnomalyEventUpdate = Updateable<AnomalyEventsTable>;

// ---------------------------------------------------------------------------
// ai_conversations (000006_outputs_and_dashboard.sql — §18.1)
// ---------------------------------------------------------------------------

/**
 * Top-level container for a user–AI dialogue session.
 *
 * PRIVACY: `metadata` may reference user context. Do not log conversation rows.
 *
 * conversation_type: 'chat' | 'sleep_summary' | 'workout_summary' |
 *   'recovery_explanation' | 'nutrition_coach'
 * status: 'active' | 'archived' | 'deleted' (soft-deleted via deleted_at)
 */
export interface AiConversationsTable {
  ...
  user_id: string;
  /**
   * Conversation context type. Enforced by DB CHECK constraint.
   * 'chat' | 'sleep_summary' | 'workout_summary' |
   * 'recovery_explanation' | 'nutrition_coach'
   */
  conversation_type: Generated<string>;
  /** Optional title (AI-generated or user-assigned). */
  title: NullableCol<string>;
  /** Conversation lifecycle status. */
  status: Generated<string>;
  created_at: CreatedAt;
  updated_at: UpdatedAt;
  /** Set when the conversation is soft-deleted. */
  deleted_at: NullableCol<Date>;
  metadata: Generated<Record<string, unknown>>;
}

/** Full row shape returned by SELECT queries on `ai_conversations`. */
export type AiConversation = Selectable<AiConversationsTable>;
/** Shape accepted by INSERT queries on `ai_conversations`. */
export type NewAiConversation = Insertable<AiConversationsTable>;
/** Shape accepted by UPDATE queries on `ai_conversations`. */
export type AiConversationUpdate = Updateable<AiConversationsTable>;

// ---------------------------------------------------------------------------
// ai_messages (000006_outputs_and_dashboard.sql — §18.2)
// ---------------------------------------------------------------------------

/**
 * Individual message turns within an AI conversation.
 *
 * PRIVACY CRITICAL:
 *   - `content` stores the actual message text, which may include health data.
 *   - Do NOT log the `content` column in application logs.
 *   - `content_redacted` is populated by the redaction pipeline for audit use.
 *
 * role: 'system' | 'user' | 'assistant' | 'tool'
 * model_provider: 'openai' | 'anthropic' | future values
 */
export interface AiMessagesTable {
  /** UUID primary key; generated by gen_random_uuid(). */
  id: UuidPk;
  /** FK to `ai_conversations.id` — CASCADE DELETE. */
   ...
   * PRIVACY: Do NOT log this field. May contain health context summaries.
   */
  content: string;
  /** Redacted copy for audit use; populated by the redaction pipeline. */
  content_redacted: NullableCol<string>;
  /** AI provider identifier. Examples: 'openai' | 'anthropic' */
  model_provider: NullableCol<string>;
  model_name: NullableCol<string>;
  prompt_tokens: NullableCol<number>;
  completion_tokens: NullableCol<number>;
  latency_ms: NullableCol<number>;
  cost_usd: NullableCol<string>;
  safety_flags: Generated<Record<string, unknown>>;
  created_at: CreatedAt;
  metadata: Generated<Record<string, unknown>>;
}

/** Full row shape returned by SELECT queries on `ai_messages`. */
export type AiMessage = Selectable<AiMessagesTable>;
/** Shape accepted by INSERT queries on `ai_messages`. */
export type NewAiMessage = Insertable<AiMessagesTable>;

// ---------------------------------------------------------------------------
// ai_context_snapshots (000006_outputs_and_dashboard.sql — §18.3)
// ---------------------------------------------------------------------------

/**
 * Structured context packets sent to or used by the AI model.
 *
 * PRIVACY CRITICAL:
 *   - `context_json` MUST contain structured summaries, deviations, and
 *     selected facts — NOT raw health payloads or unlimited observation arrays.
 *   - This constraint is enforced by application logic, not the DB schema.
 *   - Do NOT log `context_json` in application logs.
 *   - Rows are subject to `ai_context_retention_mode` in
 *     `data_retention_preferences`.
 *
 * context_type: 'chat_health_context' | 'daily_summary' | 'sleep_summary' |
 *   'workout_summary' | 'nutrition_context'
 */
export interface AiContextSnapshotsTable {
  /** UUID primary key; generated by gen_random_uuid(). */
  id: UuidPk;
  /** FK to `users.id`. */
  user_id: string;
  /** FK to `ai_conversations.id`; null for system-generated snapshots. */
  conversation_id: NullableCol<string>;
  /** FK to `ai_messages.id`; null for pre-generation snapshots. */
  message_id: NullableCol<string>;
  /**
   * Context packet type.
   * 'chat_health_context' | 'daily_summary' | 'sleep_summary' |
   * 'workout_summary' | 'nutrition_context'
   */
  context_type: string;
  context_version: string;
  local_date: NullableCol<string>;
  window_start_utc: NullableCol<Date>;
  window_end_utc: NullableCol<Date>;
  /**
   * Structured health context.
   * PRIVACY: Summaries and deviations only. Do NOT log this column.
   */
  context_json: Record<string, unknown>;
  source_score_snapshot_ids: Generated<string[]>;
  source_insight_ids: Generated<string[]>;
  source_metric_codes: Generated<string[]>;
  token_estimate: NullableCol<number>;
  retention_until: NullableCol<Date>;
  created_at: CreatedAt;
}

/** Full row shape returned by SELECT queries on `ai_context_snapshots`. */
export type AiContextSnapshot = Selectable<AiContextSnapshotsTable>;
/** Shape accepted by INSERT queries on `ai_context_snapshots`. */
export type NewAiContextSnapshot = Insertable<AiContextSnapshotsTable>;

// ---------------------------------------------------------------------------
// ai_model_invocations (000006_outputs_and_dashboard.sql — §18.5)
// ---------------------------------------------------------------------------

/**
 * Cost and usage tracking for AI model calls.
 *
 * Independent of conversations — can be joined via `metadata.message_id` when
 * needed. Primarily used for billing, auditing, and provider analytics.
 *
 * invocation_type: 'chat' | 'summary' | 'classification' | 'extraction' |
 *   'embedding'
 * status: 'succeeded' | 'failed' | 'fallback_used'
 * provider: 'openai' | 'anthropic' | future values
 */
export interface AiModelInvocationsTable {
  /** UUID primary key; generated by gen_random_uuid(). */
  id: UuidPk;
  /** FK to `users.id`; null for system-level invocations. */
  user_id: NullableCol<string>;
  /** AI provider identifier. Examples: 'openai' | 'anthropic' */
  provider: string;
  model_name: string;
  ...
  /** SHA-256 hash of the request payload (for deduplication/caching). */
  request_hash: NullableCol<string>;
  /** SHA-256 hash of the response payload. */
  response_hash: NullableCol<string>;
  latency_ms: NullableCol<number>;
  prompt_tokens: NullableCol<number>;
  completion_tokens: NullableCol<number>;
  total_cost_usd: NullableCol<string>;
  /** status: succeeded | failed | fallback_used */
  status: string;
  error_code: NullableCol<string>;
  created_at: CreatedAt;
...
/** Full row shape returned by SELECT queries on `ai_model_invocations`. */
export type AiModelInvocation = Selectable<AiModelInvocationsTable>;
/** Shape accepted by INSERT queries on `ai_model_invocations`. */
...
/**
 * User widget configuration for a dashboard screen.
 *
 * One row per (user_id, dashboard_code, widget_type) — unique constraint.
 * Ordered by `display_order` ascending for home widget rendering.
 *
 * dashboard_code: 'home' (Phase D default; additional codes in later phases)
 * widget_type: 'recovery_score' | 'sleep_score' | 'steps_ring' | 'calories' |
 *   'ai_recommendation' | 'hrv_trend' | 'bedtime_planner'
 * size: 'small' | 'medium' | 'large'
 *
 * Default rows should be seeded via a seed script, not the migration.
 */
export interface DashboardWidgetsTable {
  ...
  /** FK to `users.id`. */
  user_id: string;
  /** Which dashboard screen this widget belongs to. Default 'home'. */
  dashboard_code: Generated<string>;
  /**
   * Widget type identifier.
   * Examples: 'recovery_score' | 'sleep_score' | 'steps_ring' | 'calories' |
   *   'ai_recommendation' | 'hrv_trend' | 'bedtime_planner'
   */
  widget_type: string;
  /** Position in the ordered widget list (ascending). */
  display_order: number;
  /** Whether this widget is shown. Hidden widgets retain their order slot. */
  is_visible: Generated<boolean>;
  /** size: small | medium | large */
  size: Generated<string>;
  /** Widget-specific configuration (position overrides, custom labels, etc.). */
  config_json: Generated<Record<string, unknown>>;
  created_at: CreatedAt;
  updated_at: UpdatedAt;
}

/** Full row shape returned by SELECT queries on `dashboard_widgets`. */
export type DashboardWidget = Selectable<DashboardWidgetsTable>;
/** Shape accepted by INSERT queries on `dashboard_widgets`. */
export type NewDashboardWidget = Insertable<DashboardWidgetsTable>;
/** Shape accepted by UPDATE queries on `dashboard_widgets`. */
export type DashboardWidgetUpdate = Updateable<DashboardWidgetsTable>;

// ---------------------------------------------------------------------------
// theme_settings (000006_outputs_and_dashboard.sql — §19.2)
// ---------------------------------------------------------------------------
 ...
 */
export interface ThemeSettingsTable {
  /** PK and FK to `users.id` (1-to-1 relationship). */
  user_id: string;
  /** mode: dark | light | system */
  ...
  /** Primary accent color hex code. Default '#6C63FF'. */
  accent_color: Generated<string>;
  /** Optional secondary accent color hex code. */
  secondary_accent_color: NullableCol<string>;
  /** Whether to reduce motion effects (accessibility). */
  reduce_motion: Generated<boolean>;
  updated_at: UpdatedAt;
}

/** Full row shape returned by SELECT queries on `theme_settings`. */
export type ThemeSettings = Selectable<ThemeSettingsTable>;
/** Shape accepted by INSERT queries on `theme_settings`. */
export type NewThemeSettings = Insertable<ThemeSettingsTable>;
/** Shape accepted by UPDATE queries on `theme_settings`. */
export type ThemeSettingsUpdate = Updateable<ThemeSettingsTable>;

// ---------------------------------------------------------------------------
// mobile_cache_manifests (000006_outputs_and_dashboard.sql — §19.3)
// ---------------------------------------------------------------------------

/**
 * Optional cache-invalidation manifest for mobile clients.
 *
 * One row per (user_id, cache_scope, scope_date). Mobile clients can poll
 * the version_hash to detect stale cached data without re-fetching payloads.
 *
 * cache_scope: 'home' | 'sleep' | 'recovery' | 'activity' | 'nutrition' |
 *   'ai_context'
 */
export interface MobileCacheManifestsTable {
  /** UUID primary key; generated by gen_random_uuid(). */
  id: UuidPk;
  /** FK to `users.id`. */
  user_id: string;
  /**
   * Which screen or data domain this manifest covers.
   * 'home' | 'sleep' | 'recovery' | 'activity' | 'nutrition' | 'ai_context'
   */
  cache_scope: string;
  /** Local date this manifest applies to; null for scope-wide manifests. */
  scope_date: NullableCol<string>;
  /** Hash identifying the current version of the cached data. */
  version_hash: string;
  generated_at: CreatedAt;
  expires_at: NullableCol<Date>;
  metadata: Generated<Record<string, unknown>>;
...
/** Full row shape returned by SELECT queries on `mobile_cache_manifests`. */
export type MobileCacheManifest = Selectable<MobileCacheManifestsTable>;
/** Shape accepted by INSERT queries on `mobile_cache_manifests`. */
export type NewMobileCacheManifest = Insertable<MobileCacheManifestsTable>;
...
export interface Database {
  ...
  // CU-031: Scores, insights, AI, and dashboard tables
  ...
  anomaly_events: AnomalyEventsTable;
